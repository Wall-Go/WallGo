...
#        )
        ...
        msq3 = msq + T**2 / 16. * (3. * g2**2 + g1**2 + 4.*yt**2 + 8.*lam) + T**2 * a2 / 24. - a23*np.sqrt(b23)/(8.0*np.pi)  
         
        lam3 = T*lam - a23**2/(32.0*np.pi*np.sqrt(b23)) 
...
#    da = 0.01
    ...
    for a2 in values_a2:
        ...
        phaseInfo = WallGo.PhaseInfo(temperature = Tn, 
					...
					phaseLocation1 = WallGo.Fields( [0.0, 0.0] ), 
