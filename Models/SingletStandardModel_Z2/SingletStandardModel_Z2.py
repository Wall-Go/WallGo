...
class SingletSM_Z2(WallGo.GenericModel):
...
class EffectivePotentialxSM_Z2(WallGo.EffectivePotential_NoResum):
    ...
    def evaluate(self, fields: WallGo.Fields, temperature: float) -> complex:
    ...
    def boson_massSq(self, fields: WallGo.Fields, temperature):
    ...
    def fermion_massSq(self, fields: WallGo.Fields, temperature):
...
def main():
    ...
    """Initialize your WallGo.GenericModel instance. 
    ...
    If you need to use multiple models during a single run, we recommend creating a separate WallGo.WallGoManager instance for each model. 
    """
    ...
    collisionFileName = (
        ...
        / "Collisions/collisions_top_top_N11.hdf5"
    )
    ...
    """ Example mass loop that just does one value of mh2. Note that the WallGo.WallGoManager class is NOT thread safe internally, 
    ...
    for mh2 in values_mh2:
        ...
        """Give the input to WallGo. It is NOT enough to change parameters directly in the WallGo.GenericModel instance because
            1) WallGo needs the PhaseInfo 
            2) WallGo.WallGoManager.setParameters() does parameter-specific initializations of internal classes
        """
        ...
        # plotting Delta00
        xi, _, _ = manager.grid.getCoordinates(endpoints=True)
        Delta00 = results.Deltas.Delta00
        fig, ax = plt.subplots(1, 1)
        ax.plot(xi, Delta00.evaluate((xi,)), "-")
        ax.set_xlabel(r"$\xi$")
        ax.set_ylabel(r"$\Delta_{00}$")
        ax.grid(True)
        plt.show()
