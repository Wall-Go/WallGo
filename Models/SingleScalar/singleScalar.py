"""
A simple example model, of a single real scalar.
Note that this model is not very physical, but designed to demonstrate a full computation
of the wall velocity with as few components as possible.
"""
...
class ScalarModel(GenericModel):
    """
    The scalar model, inheriting from WallGo.GenericModel.
    """

    def __init__(self) -> None:
        """
        Initialize the scalar model.
        ...
        ----------
        cls: ScalarModel
            An object of the ScalarModel class.
        """

        self.modelParameters: dict[str, float] = {}

        # Initialize internal effective potential
        self.effectivePotential = EffectivePotentialScalar(self)
    ...
    def getEffectivePotential(self) -> "EffectivePotentialScalar":
    ...
    def defineParticles(self) -> None:
        ...
        # points in fields.
        def hMsqVacuum(fields: Fields) -> Fields:
            return self.modelParameters["msq"]+self.modelParameters["gam"]* fields.getField(0) + self.modelParameters[
                "lam"
            ] * fields.getField(0)**2/2
        ...
        def hMsqDerivative(fields: Fields) -> Fields:  # pylint: disable = W0613
            return 6*self.modelParameters["lam"]* fields.getField(0) + 15 * self.modelParameters[
                "kappa"
            ] * fields.getField(0)**3

        def hMsqThermal(T: float) -> float:
