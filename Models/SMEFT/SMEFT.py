...
#        modelParameters["msq"] = -modelParameters["lambda"] * v0**2
        ...
        #M = 650
        c6 = 1/M**2
        modelParameters["c6"] = c6

        modelParameters["lambda"] = 0.5 * mH**2 / v0**2 -  (3.0/2.0)*c6*v0**2
        modelParameters["msq"] = -mH**2 / 2. + (3.0/4.0)*c6*v0**4
...
class EffectivePotentialSM(EffectivePotential):
    ...
    def evaluate(self, fields: Fields, temperature: float) -> complex:
        ...
        #M = 650
        #c6 = 1/M**2
        c6 = thermalParameters["c6"]

        # tree level potential
        #V0 = 0.5 * msq * v**2 + 0.25 * lam * v**4
        V0dim6 = 0.5 * msq * v**2 + 0.25 * lam * v**4 + 0.125 * c6 * v**6
        ...
        VTotal = V0dim6 + self.constantTerms(T)
...
def main():
   ...
   ## ---- This is where you'd start an input parameter loop if doing parameter-space scans ----
    ...
    for mH in values_mH:
        ...
        a = modelParameters["g1"]
        b = modelParameters["g2"]
        c = modelParameters["yt"]

        g1sq = a*a
        g2sq = b*b
        ytsq = c*c

        print(f"\nPeek2: {g1sq} \t {g2sq} \t {ytsq} \n")
