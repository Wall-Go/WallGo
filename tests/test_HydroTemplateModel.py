...
def test_JouguetVelocity():
    ...
    for i in range(N):
        model = TestModel.TestModelTemplate(alN[i],psiN[i],cb2[i],cs2[i],1,1)
        hydro = Hydro(model)
        hydroTemplate = HydroTemplateModel(model)
...
def test_findMatching():
    ...
    for i in range(N):
        model = TestModel.TestModelTemplate(alN[i],psiN[i],cb2[i],cs2[i],1,1)
        hydro = Hydro(model,1e-10,1e-10)
        hydroTemplate = HydroTemplateModel(model,1e-10,1e-10)
