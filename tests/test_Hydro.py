...
model1 = TestModel.TestModel2Step(0.2,0.1,0.4,1)
hydro = Hydro(model1)
...
def test_LTE():
    res = np.zeros(5)
    for i in range(5):
        model2 = TestModel.TestModelBag(0.9,0.5+i*0.1)        
        hydro2 = Hydro(model2)
    ...
    res2 = np.zeros(4)
    for i in range(4):
        model2 =     model2 = TestModel.TestModelBag(0.8,0.6+i*0.1)
        hydro2 = Hydro(model2)
