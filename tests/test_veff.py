...
import numpy as np
import WallSpeed
from examples.models.xSM import xSM
...
def test_BM1():
    ...
    Tplus = 103.1
    Tminus = 100.1
    mod = xSM(125, 120, 1.0, 0.9)
    res = mod.evaluateVeff([[110],[130]], 100)
    assert res.shape == (1,)
    assert res == pytest.approx([-1.19018205e09], rel=1e-2)
    free = WallSpeed.FreeEnergy(mod, Tc, Tnucl=Tn)
...
def test_BM2():
    mod = xSM(125, 160.0, 1.0, 1.2)
    res = mod.evaluateVeff([[100,110],[130,130]], 100)
    assert res.shape == (2,)
    np.testing.assert_allclose(
...
def test_BM4():
    mod = xSM(125, 80, 1.0, 0.5)
    res = mod.evaluateVeff([[100, 110],[100, 130]], 100)
    np.testing.assert_allclose(
