...
class FreeEnergyValueType:
    r"""
    Return type for FreeEnergy, holding both :math:`V(x)` and :math:`x`.
    """

    veffValue: np.ndarray
    r"""Value of the effective potential (free energy) at a minimum"""

    fieldsAtMinimum: Fields
    r"""Value of the fields at a minimum"""

    @staticmethod
    def fromArray(arr: np.ndarray) -> "FreeEnergyValueType":
        """Converts array to FreeEnergyValueType.
        Assumes that the last column is Veff value,
        and previous columns are field values.
        """
...
class FreeEnergy(InterpolatableFunction):
    """Class describing properties of a phase, a local minimum of an effective potential.
    ...
    def evaluate(
        self, x: np.ndarray, bUseInterpolatedValues=True
    ) -> FreeEnergyValueType:
    ...
    def _functionImplementation(self, temperature: np.ndarray) -> np.ndarray:
        """
    ...
    def derivative(
        self, x: np.ndarray, order: int = 1, bUseInterpolation=True
    ) -> FreeEnergyValueType:
    ...
    def tracePhaseOld(self, TMin: float, TMax: float, dT: float) -> None:
        ...
        if not self.hasInterpolation():
            ...
            self.extendInterpolationTable(
                TMin, TMax, math.ceil(numPoints / 2), math.ceil(currentPoints / 2)
            )
        ...
        for direction in [0, 1]:
            ...
            while ode.status == "running":
                ...
                if ode.step_size < 1e-16 * T0:
                    print(
                        f"Step size {ode.step_size} shrunk too small at T={ode.t}, vev={ode.y}"
                    )
                    break
            if direction == 0:
                ...
                if len(TList) > 1:
                    ...
                    VeffFullList = np.append(
                        np.flip(VeffList, axis=0), VeffFullList, axis=0
                    )
